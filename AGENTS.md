...
│   │   └── MapInfo.java                   # SLAM 地图实体（pgm/yaml 元数据）
...
- **启动加载**：`@PostConstruct` 优先加载数据库中 `is_active=1` 的地图；若无则自动导入 `slam/guli.pgm` + `slam/guli.yaml` 作为默认地图。
...
5. **路径规划说明**：
   - `SLAMServiceImpl` 已实现 ROS 标准 `.pgm`/`.yaml` 解析与 MySQL 持久化，支持多地图切换。
   - 新增 `map_live` 表用于实时地图快照（栅格 + 障碍物），RosBridge 推送地图或用户修改障碍物时自动落库，重启后可恢复。
   - `RobotServiceImpl.generatePath()` 仍仅做线性插值，非真实导航路径（如需真实路径请调用 `SLAMService.planPath`）。
