...
项目不包含任何单元测试或集成测试代码（`src/test` 目录不存在），但 `pom.xml` 已引入 `spring-boot-starter-test`。
...
│   │   ├── LLMService.java
│   │   ├── SLAMService.java
│   │   ├── RosBridgeService.java          # ROS2 WebSocket 通信接口
...
│       ├── RosBridgeServiceImpl.java      # rosbridge WebSocket 客户端（订阅 /map、/amcl_pose，发布 /goal_pose）
...
- 配置项：`data-service.url`、`data-service.retry.max-attempts`、`data-service.retry.backoff-multiplier`。
...
- **实时地图持久化**：`map_live` 表保存当前内存中的实时栅格（含障碍物）与障碍物列表，服务重启后可自动恢复上次状态；RosBridge 收到 `/map` 话题数据或障碍物发生变更时自动写入。
- **A* 路径规划**：8 方向搜索，基于当前内存栅格（含障碍物），带简单路径平滑。

### 6.7 ROS2 通信对接（RosBridgeServiceImpl）
- WebSocket 客户端连接 `rosbridge_server`（默认 `ws://localhost:9090`）。
- 自动订阅 `/map`（`nav_msgs/OccupancyGrid`）→ 解析后调用 `slamService.updateMapData()` 更新内存栅格并自动持久化到 `map_live`。
- 自动订阅 `/amcl_pose`（`geometry_msgs/PoseWithCovarianceStamped`）→ 解析位姿后更新对应机器人数据库记录。
- 支持通过 `/goal_pose` 向 ROS2 发送真实导航目标；调用前会先更新内存目标与 Mock 路径。
- 自带 30 秒健康检查与 10 秒重连冷却，避免频繁重连。

### 6.8 动态优先级规划（TaskPriorityPlannerImpl）
...
2. **无测试代码**：项目中没有任何 `src/test` 目录或测试类（`pom.xml` 已引入 `spring-boot-starter-test` 供后续扩展）。
